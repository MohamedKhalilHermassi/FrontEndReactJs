...
const SessionEdit = () => {
  ...
  useEffect(() => {
    const fetchSession = async () => {
      try {
        const sessionResponse = await fetch(`https://backendexpressjs-2.onrender.com/sessions/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`https://backendexpressjs-2.onrender.com/sessions/${id}`, {
        method: 'PUT',
        headers: {
