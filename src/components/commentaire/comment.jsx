...
export default function Comment(props) {
  ...
  const addReplyToComment = async (text) => {
    try {
      ...
      // Update frontend state to display the newly added reply
      setBackendReplies([...backendReplies, data.reply]);
    } catch (error) {
  ...
  return (
    ...
    <NewComment
  currentUser={props.currentUser}
  placeholder={`Replying to @${props.user.fullname}`}
  handleSubmit={(text) => addReplyToComment(text)}
  buttonText='reply'
/>
        </div>
      )}
